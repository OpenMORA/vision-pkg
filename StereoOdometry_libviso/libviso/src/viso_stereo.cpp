...
bool VisualOdometryStereo::process (uint8_t *I1,uint8_t *I2,int32_t* dims,bool replace) {
  ...
  // JLBC: 
  // Discard the effects of disparity misalignment: 
  {
	  const size_t N = p_matched.size();
	  for (size_t i=0;i<N;i++)
	  {
		  // Replace "v" coordinate with an average of left/right:
		  const float vc_avrg = .5f* (p_matched[i].v1c + p_matched[i].v2c );
		  const float vp_avrg = .5f* (p_matched[i].v1p + p_matched[i].v2p );

		  p_matched[i].v1c = vc_avrg; p_matched[i].v2c = vc_avrg;
		  p_matched[i].v1p = vp_avrg; p_matched[i].v2p = vp_avrg;
	  }
  }
...
vector<double> VisualOdometryStereo::estimateMotion (vector<Matcher::p_match> p_matched) {
  ...
  if (inliers.size()>=6) {
    ...
    VisualOdometryStereo::result result = UPDATED;
    while (result==UPDATED) {     
      result = updateParameters(p_matched,inliers,tr_delta,1,1e-8); // 1e-8
