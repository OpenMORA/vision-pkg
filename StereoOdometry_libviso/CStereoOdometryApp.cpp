...
bool CStereoOdometryApp::OnStartUp()
{
	...
	//! @moos_param FORZE_2D (Default=false) Forze Z coordinate, Pitch & Roll to be always exactly zero
	m_forze_2d = m_ini.read_bool("","FORZE_2D", false );
...
void CStereoOdometryApp::processVisualStereoOdometry( const mrpt::slam::CObservationStereoImagesPtr &obs)
{
	...
	if (m_viso->process( left_gray(0,0), right_gray(0,0), dims, img_buf_replace ))
	{
		...
		if (m_forze_2d)
			current_kf = mrpt::poses::CPose3D( mrpt::poses::CPose2D(current_kf ) );
		...
		if (SHOW_GUI)
		{
			if (!m_gui)
			{
                ...
                gl_ptns = mrpt::opengl::CPointCloudColoured::Create();
                gl_ptns->setPointSize(3.0);
                scene->insert(gl_ptns);
			...
			// Visualize matches: as 2D lines and as 3D points:
			const std::vector<Matcher::p_match> visualMatches = m_viso->getMatches();

			gl_ptns->clear();
			gl_ptns->setPose(m_odometry_accum_pose + obs->cameraPose );
			...
			for (size_t i=0;i<visualMatches.size();i++)
			{
                ...
                // Point 3D:
                const double base = m_viso->param.base;
                const double cu = m_viso->param.calib.cu, cv = m_viso->param.calib.cv, f = m_viso->param.calib.f;
                const double d = m.u1c - m.u2c;

                const float x = (m.u1c-cu)*base/d;
                const float y = (m.v1c-cv)*base/d;
                const float z = f*base/d;
                const float R = 1;
                const float G = 1;
                const float B = 1;

                gl_ptns->push_back(x,y,z,R,G,B);
			}
