...
public:
  ...
  // JLBC for OpenMORA:
  /** Like getMotion but returns [rx ry rz tx ty tz]  */
  const std::vector<double> & getMotionVec() const { return Tr_delta_vec; }
...
protected:
  ...
  std::vector<double>            Tr_delta_vec; //!< transformation (previous -> current frame)   [rx ry rz tx ty tz]  // JLBC for OpenMORA
